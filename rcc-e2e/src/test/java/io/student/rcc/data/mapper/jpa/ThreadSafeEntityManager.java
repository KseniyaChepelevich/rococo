...
public class ThreadSafeEntityManager implements EntityManager {
    ...
    public ThreadSafeEntityManager(EntityManagerFactory emf) {
        this.emf = emf;
        this.threadEm = ThreadLocal.withInitial(this.emf::createEntityManager);
    }
    ...
    public boolean isOpen() {
        // ИСПРАВЛЕНО: Метод теперь проверяет состояние конкретного потока, а не зацикливается сам на себе
        EntityManager em = threadEm.get();
