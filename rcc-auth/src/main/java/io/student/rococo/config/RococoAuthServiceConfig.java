...
//  }
  ...
  public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http) throws Exception {
      // 1. Создаем конфигуратор сервера авторизации
      OAuth2AuthorizationServerConfigurer authorizationServerConfigurer =
              new OAuth2AuthorizationServerConfigurer();

      // 2. Настраиваем OIDC и логаут (через лямбды)
      ...
      // 3. Применяем настройки к http
      http
              .with(authorizationServerConfigurer, (authorizationServer) -> {
                  // Устанавливаем мачер эндпоинтов сервера авторизации
                  http.securityMatcher(authorizationServer.getEndpointsMatcher());
              })
              // 4. Остальные настройки в стиле Spring Security 6
